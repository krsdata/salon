<?php
  ...
  if (mysqli_connect_errno())
    ...
    WHERE
      ...
      mss_customer_packages.customer_id=mss_customers.customer_id AND
      mss_customers.customer_business_outlet_id=mss_business_outlets.business_outlet_id AND
      mss_auto_engage.business_admin_id= mss_customers.customer_business_admin_id AND
      mss_auto_engage.business_outlet_id= mss_customers.customer_business_outlet_id AND
      mss_customer_packages.salon_package_id=mss_salon_packages.salon_package_id AND
      date(mss_customer_packages.package_expiry_date)=date(now())+ INTERVAL 10 DAY
    GROUP BY
    mss_customer_packages.customer_package_id" ;

    $result = $con->query($sql);
    // print_r($result);
    // echo date('Y-m-d',strtotime('+6 day'));
    // exit;
    while($row=$result->fetch_assoc()){
     print_r(SendPackageExpirySms("8454801188",$row['customer_name'],$row['business_outlet_location'],$row['business_outlet_name'],$row['business_outlet_sender_id'],$row['api_key'],$row['package_expiry_date'],$row['salon_package_name']));
    ...
    //
    function SendPackageExpirySms($mobile,$name,$location,$outlet_name,$sender_id,$api_key,$expiry_date,$package_name){
		//API key & sender ID
