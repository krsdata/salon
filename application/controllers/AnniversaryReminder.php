  ...
  if (mysqli_connect_errno())
    ...
    $sql="SELECT
		...
		WHERE
			...
			EXTRACT(MONTH FROM mss_customers.customer_doa)= EXTRACT(MONTH FROM CURRENT_DATE) AND
			EXTRACT(DAY FROM mss_customers.customer_doa) = EXTRACT(DAY FROM CURRENT_DATE) AND
    ...
    while($row=$result->fetch_assoc()){
			if($row['trigger_type']=='Anniversary'){
    ...
    function SendAutometicAppointmentSms($mobile,$name,$location,$outlet_name,$sender_id,$api_key,$offer){
		...
		// $apisender = "BILLIT";
		$msg = "Happy Anniversary Dear ".$name." FROM ".$outlet_name.",".$location.". GRAB flat ".$offer." off on your next visit."." Offer valid till ".date('Y-m-d', strtotime("+1 months", strtotime("NOW"))).".";
