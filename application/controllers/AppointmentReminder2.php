<?php 
  ...
  FROM
    ...
    mss_appointments
  WHERE
    mss_auto_engage.trigger_type='Appointment_Reminder' AND
    mss_appointments.appointment_status=1 AND
    ...
    mss_appointments.customer_id=mss_customers.customer_id AND
    mss_customers.customer_business_outlet_id=mss_business_outlets.business_outlet_id AND
    mss_appointments.appointment_date=date(now())+ INTERVAL 1 DAY
  GROUP BY
    ...
    while($row=$result->fetch_assoc()){
     SendAutometicAppointmentSms(8454801188,$row['customer_name'],$row['business_outlet_location'],$row['business_outlet_name'],$row['business_outlet_sender_id'],$row['api_key'],$row['appointment_date'],$row['appointment_start_time']);
